...
| `undrifted_issue01_ai_isnt_broken_systems_are_article_v1` | article | ready_for_publication | undrifted_issue01 | unDrifted/Issue01 (cover_story) | oar2_register_undrifted_issue001_cover_story_ai_isnt_broken_systems_are_v1 | Assets/Articles/unDrifted/Issue01/registered/undrifted_issue01_ai_isnt_broken_systems_are_article_v1.md |
...
The cover story article (`undrifted_issue01_ai_isnt_broken_systems_are_article_v1`) was revised from `registered_draft` (short-form, no long-form draft available at registration time) to `ready_for_publication` once the operator supplied the full approved draft directly. Body replaced verbatim — no content added or altered beyond what was supplied. **Not yet published to Paragraph or bound into the live `/undrifted` feature selection** — both remain separate, explicit decisions, matching the pattern used for the Editor's Letter (registered first, published only after an explicit confirmation). Its banner (`undrifted_ai_isnt_broken_landing_banner_v1`) binds to the pre-existing `ai_isnt_broken_landing.webp` media asset already live as the `/undrifted` cover image — no binary was duplicated or newly uploaded.
