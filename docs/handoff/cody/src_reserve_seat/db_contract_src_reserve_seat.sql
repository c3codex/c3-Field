-- =====================================================
-- SRC INTAKE REQUEST + OAR1 LOG
-- Measures Registry — Reserve Seat Intake
-- =====================================================

create extension if not exists pgcrypto;

create table if not exists public.src_intake_request (
...
create table if not exists public.oar1_log (
  ...
  src_intake_request_id uuid not null
    references public.src_intake_request(id)
    on delete cascade,
...
create or replace function public.submit_src_intake_request(
  p_origin_type text,
  p_full_name text,
  p_email text,
...
declare
  ...
  v_env_key uuid;
begin
  insert into public.src_intake_request (
    origin_type,
    full_name,
    email,
    role_or_title,
  ...
  values (
    ...
    'SRC intake request received from landing page and envKey generated',
    'SRC intake standing established; awaiting operator review',
    'measures_registry_landing',
    'measures_registry_june_cohort',
    v_env_key
  );

  update public.src_intake_request
...
alter table public.src_intake_request enable row level security;
alter table public.oar1_log enable row level security;

drop policy if exists "service role manages src intake" on public.src_intake_request;
drop policy if exists "service role manages oar1" on public.oar1_log;

create policy "service role manages src intake"
on public.src_intake_request
for all
...
grant execute on function public.submit_src_intake_request(
