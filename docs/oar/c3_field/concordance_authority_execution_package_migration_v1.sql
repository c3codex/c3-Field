-- Concordance Authority Execution Package Migration v1
...
for each row execute function public.concordance_authority_set_updated_at();
...
for each row execute function public.concordance_authority_prevent_mutation();
...
before update on public.concordance_version
for each row execute function public.concordance_authority_protect_recognition_timestamps();
...
using (authority_standing = 'active' and visibility_standing = 'public');
...
for select
to anon, authenticated
using (version_standing = 'active' and visibility_standing = 'public');
