...
async function readBack() {
  const documentCount = await countRows("concordance_document", (query) =>
    query
      .eq("document_key", "concordance_authority_seating_system_intelligence")
      .eq("authority_standing", "active")
      .eq("visibility_standing", "internal")
      .eq("document_scope", "semantic_infrastructure")
      .eq("metadata->>document_type", "system_intelligence")
      .eq("metadata->>authority_level", "system")
      .eq("metadata->>status", "seeded_candidate"),
  )

  const versionCount = await countRows("concordance_version", (query) =>
    query
      .eq("version_key", "concordance_authority_seating_system_intelligence_v1")
      .eq("document_key", "concordance_authority_seating_system_intelligence")
  ...
  const relationCount = await countRows("concordance_relation", (query) =>
    query
      ...
      .eq("visibility_standing", "internal")
      .eq("metadata->>seating", "system_intelligence_capture"),
  )

  const termCount = await countRows("concordance_term", (query) =>
    query.eq("version_key", "concordance_authority_seating_system_intelligence_v1"),
  )

  const snapshotCount = await countRows("seeded_source_snapshot", (query) =>
    query
      .eq("snapshot_key", "concordance_authority_system_intelligence_v1_local_source_20560e6a")
      .eq("version_key", "concordance_authority_seating_system_intelligence_v1")
  ...
  const readback = {
    systemIntelligenceDocuments: documentCount,
    systemIntelligenceVersions: versionCount,
    systemIntelligenceRelations: relationCount,
    systemIntelligenceTerms: termCount,
    verifiedSnapshots: snapshotCount,
  }

  console.log(`system_intelligence_capture_readback: ${JSON.stringify(readback)}`)

  if (documentCount !== 1) throw new Error("system intelligence document not seated")
  if (versionCount !== 1) throw new Error("system intelligence version not seated")
  if (relationCount !== 16) {
    throw new Error("expected 16 system intelligence capture relations")
  }
  if (termCount !== 0) throw new Error("system intelligence seating must not create terms")
...
async function main() {
  ...
  await execSqlFile(files.seating, "phase_1_system_intelligence_capture_seating")
  await execSqlFile(files.validation, "phase_2_system_intelligence_capture_validation_sql")
  await readBack()
}
