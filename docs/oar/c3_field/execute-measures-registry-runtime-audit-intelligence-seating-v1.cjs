...
const expectedByteSize = 30318

const files = {
  seating:
    "docs/oar/c3_field/measures_registry_runtime_audit_intelligence_seating_v1.sql",
  validation:
    "docs/oar/c3_field/measures_registry_runtime_audit_intelligence_validation_v1.sql",
}
...
async function readBack() {
  const documentCount = await countRows("concordance_document", (query) =>
    query
      .eq("document_key", "measures_registry_runtime_audit_intelligence")
      .eq("authority_standing", "active")
      .eq("visibility_standing", "internal")
      .eq("document_scope", "runtime_governance")
      .eq("metadata->>document_type", "system_intelligence")
      .eq("metadata->>status", "seeded")
      .eq("metadata->>system_key", "c3field")
      .eq("metadata->>intelligence_class", "implementation_intelligence"),
  )

  const versionCount = await countRows("concordance_version", (query) =>
    query
      .eq("version_key", "measures_registry_runtime_audit_intelligence_v1")
      .eq("document_key", "measures_registry_runtime_audit_intelligence")
  ...
  const relationCount = await countRows("concordance_relation", (query) =>
    query
      ...
      .eq("visibility_standing", "internal")
      .eq("metadata->>seating", "runtime_governance_intelligence_capture"),
  )

  const snapshotCount = await countRows("seeded_source_snapshot", (query) =>
    query
      .eq("snapshot_key", "mrs_runtime_audit_intelligence_v1_local_source_2e2daffb")
      .eq("version_key", "measures_registry_runtime_audit_intelligence_v1")
  ...
  const readback = {
    intelligenceDocuments: documentCount,
    intelligenceVersions: versionCount,
    intelligenceRelations: relationCount,
    verifiedSnapshots: snapshotCount,
  }

  console.log(`runtime_audit_intelligence_readback: ${JSON.stringify(readback)}`)

  if (documentCount !== 1) throw new Error("intelligence document not seated")
  if (versionCount !== 1) throw new Error("intelligence version not seated")
  if (relationCount !== 19) {
    throw new Error(`expected 19 intelligence relations, got ${relationCount}`)
  }
...
async function main() {
  ...
  await execSqlFile(files.seating, "phase_1_runtime_audit_intelligence_seating")
  await execSqlFile(files.validation, "phase_2_runtime_audit_intelligence_validation_sql")
  await readBack()
}
