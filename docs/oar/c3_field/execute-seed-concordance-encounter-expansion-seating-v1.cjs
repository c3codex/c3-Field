...
async function readBack() {
  ...
  const relationCount = await countRows("concordance_relation", (query) =>
    query
      ...
      .eq("metadata->>expansion", "seed_concordance_encounter_expansion_seating_v1"),
  )

  console.log(
    `encounter_expansion_readback: ${JSON.stringify({
      termCount,
      relationCount,
    })}`,
  )
  ...
  if (termCount !== 6) {
    throw new Error(`encounter validation failed: expected 6 terms, got ${termCount}`)
  }
  if (relationCount !== 41) {
    throw new Error(
      `encounter validation failed: expected 41 relations, got ${relationCount}`,
    )
  }
}
...
async function main() {
  ...
  await execSqlFile(files.seating, "phase_1_encounter_expansion_seating")
  await execSqlFile(files.validation, "phase_2_encounter_expansion_validation_sql")
  await readBack()
}
