...
async function readBack() {
  ...
  const relationCount = await countRows("concordance_relation", (query) =>
    query
      ...
      .eq("metadata->>expansion", "seed_concordance_foundational_coherence_set_seating_v1"),
  )

  console.log(
    `foundational_coherence_set_readback: ${JSON.stringify({
      termCount,
      relationCount,
    })}`,
  )
  ...
  if (termCount !== 6) {
    throw new Error(`coherence set validation failed: expected 6 terms, got ${termCount}`)
  }
  if (relationCount !== 50) {
    throw new Error(
      `coherence set validation failed: expected 50 relations, got ${relationCount}`,
    )
  }
}
...
async function main() {
  ...
  await execSqlFile(files.seating, "phase_1_foundational_coherence_set_seating")
  await execSqlFile(files.validation, "phase_2_foundational_coherence_set_validation_sql")
  await readBack()
}
