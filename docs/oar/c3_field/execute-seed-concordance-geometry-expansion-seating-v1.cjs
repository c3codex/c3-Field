...
async function readBack() {
  ...
  const relationCount = await countRows("concordance_relation", (query) =>
    query
      ...
      .eq("metadata->>expansion", "seed_concordance_geometry_expansion_seating_v1"),
  )

  console.log(
    `geometry_expansion_readback: ${JSON.stringify({
      termCount,
      relationCount,
    })}`,
  )
  ...
  if (termCount !== 6) {
    throw new Error(`geometry validation failed: expected 6 terms, got ${termCount}`)
  }
  if (relationCount !== 35) {
    throw new Error(
      `geometry validation failed: expected 35 relations, got ${relationCount}`,
    )
  }
}
...
async function main() {
  ...
  await execSqlFile(files.seating, "phase_1_geometry_expansion_seating")
  await execSqlFile(files.validation, "phase_2_geometry_expansion_validation_sql")
  await readBack()
}
