...
async function readBack() {
  const documentCount = await countRows("concordance_document", (query) =>
    query
      .eq("document_key", "seed_concordance_governance_usage_change_control")
      .eq("authority_standing", "active")
      .eq("visibility_standing", "internal")
      .eq("document_scope", "semantic_governance")
      .eq("metadata->>document_type", "concordance_governance")
      .eq("metadata->>authority_level", "system")
      .eq("metadata->>status", "readonly_candidate"),
  )

  const versionCount = await countRows("concordance_version", (query) =>
    query
      .eq("version_key", "seed_concordance_governance_usage_change_control_v1")
      .eq("document_key", "seed_concordance_governance_usage_change_control")
  ...
  const relationCount = await countRows("concordance_relation", (query) =>
    query
      ...
      .eq("visibility_standing", "internal")
      .eq("metadata->>seating", "governance_posture"),
  )

  const termCount = await countRows("concordance_term", (query) =>
    query.eq("version_key", "seed_concordance_governance_usage_change_control_v1"),
  )
  ...
  const readback = {
    governanceDocuments: documentCount,
    governanceVersions: versionCount,
    governanceRelations: relationCount,
    governanceTerms: termCount,
    verifiedSnapshots: snapshotCount,
  }

  console.log(`governance_change_control_readback: ${JSON.stringify(readback)}`)

  if (documentCount !== 1) throw new Error("governance document not seated")
  if (versionCount !== 1) throw new Error("governance version not seated")
  if (relationCount !== 8) throw new Error("expected 8 governance posture relations")
  if (termCount !== 0) throw new Error("governance seating must not create terms")
...
async function main() {
  ...
  await execSqlFile(files.seating, "phase_1_governance_change_control_seating")
  await execSqlFile(files.validation, "phase_2_governance_change_control_validation_sql")
  await readBack()
}
