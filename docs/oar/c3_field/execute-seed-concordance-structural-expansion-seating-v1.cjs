...
async function readBack() {
  const termCount = await countRows("concordance_term", (query) =>
    query
      ...
      .in("term_label", structuralLabels),
  )
  ...
  const relationCount = await countRows("concordance_relation", (query) =>
    query
      ...
      .eq("metadata->>expansion", "seed_concordance_structural_expansion_seating_v1"),
  )

  console.log(
    `structural_expansion_readback: ${JSON.stringify({
      termCount,
      relationCount,
    })}`,
  )
  ...
  if (termCount !== 15) {
    throw new Error(`structural validation failed: expected 15 terms, got ${termCount}`)
  }
  if (relationCount !== 67) {
    throw new Error(
      `structural validation failed: expected 67 relations, got ${relationCount}`,
    )
  }
}
...
async function main() {
  ...
  await execSqlFile(files.seating, "phase_1_structural_expansion_seating")
  await execSqlFile(files.validation, "phase_2_structural_expansion_validation_sql")
  await readBack()
}
