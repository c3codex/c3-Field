-- c3 Key Assign Temporary System Function Source Binding Hardening v1
-- Source OAR2: docs/oar/measures_interoperability/oar2_c3_key_assign_temporary_system_function_source_binding_hardening_v1.meta.md
--
-- This seats explicit source-record / OAR binding verification for
-- assign_temp_c3_key. It does not issue keys by itself, grant permissions,
...
CREATE TABLE IF NOT EXISTS public.c3_key_source_oar_binding (
  ...
  CONSTRAINT c3_key_source_oar_binding_source_record_id_not_blank_check
    CHECK (length(btrim(source_record_id)) > 0),
  CONSTRAINT c3_key_source_oar_binding_source_oar_id_not_blank_check
    CHECK (length(btrim(source_oar_id)) > 0),
  CONSTRAINT c3_key_source_oar_binding_source_oar_path_not_blank_check
    CHECK (source_oar_path IS NULL OR length(btrim(source_oar_path)) > 0),
  CONSTRAINT c3_key_source_oar_binding_operator_ref_not_blank_check
    CHECK (length(btrim(operator_ref)) > 0),
  CONSTRAINT c3_key_source_oar_binding_metadata_object_check
    CHECK (jsonb_typeof(metadata) = 'object'),
  CONSTRAINT c3_key_source_oar_binding_metadata_support_safe_check
    CHECK (
      NOT (
        metadata ?| ARRAY[
          'temp_key',
  ...
  CONSTRAINT c3_key_source_oar_binding_unique_source_oar
    UNIQUE (source_record_type, source_record_id, source_oar_id)
);

ALTER TABLE public.c3_key_source_oar_binding ENABLE ROW LEVEL SECURITY;

REVOKE ALL ON TABLE public.c3_key_source_oar_binding FROM PUBLIC;
REVOKE ALL ON TABLE public.c3_key_source_oar_binding FROM anon;
REVOKE ALL ON TABLE public.c3_key_source_oar_binding FROM authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public.c3_key_source_oar_binding TO service_role;

CREATE INDEX IF NOT EXISTS c3_key_source_oar_binding_source_idx
  ON public.c3_key_source_oar_binding (source_record_type, source_record_id);

CREATE INDEX IF NOT EXISTS c3_key_source_oar_binding_oar_idx
  ON public.c3_key_source_oar_binding (source_oar_id);

CREATE INDEX IF NOT EXISTS c3_key_source_oar_binding_active_idx
  ON public.c3_key_source_oar_binding (
    source_record_type,
    source_record_id,
    source_oar_id,
    binding_status
...
CREATE OR REPLACE FUNCTION public.touch_c3_key_source_oar_binding_updated_at()
RETURNS trigger
LANGUAGE plpgsql
...
DROP TRIGGER IF EXISTS trg_touch_c3_key_source_oar_binding_updated_at
ON public.c3_key_source_oar_binding;

CREATE TRIGGER trg_touch_c3_key_source_oar_binding_updated_at
BEFORE UPDATE ON public.c3_key_source_oar_binding
FOR EACH ROW
EXECUTE FUNCTION public.touch_c3_key_source_oar_binding_updated_at();

CREATE OR REPLACE FUNCTION public.get_c3_key_source_oar_binding_support_read(
...
AS $$
  SELECT
    b.id,
    b.source_record_type,
    b.source_record_id,
    b.source_oar_id,
    b.binding_status,
    (b.binding_status = 'active' AND b.support_safe IS TRUE) AS has_active_binding,
    b.audit_id,
    b.created_at
  FROM public.c3_key_source_oar_binding b
  WHERE b.source_record_type = p_source_record_type
    AND b.source_record_id = p_source_record_id
    AND b.source_oar_id = p_source_oar_id
    AND b.support_safe IS TRUE
  ORDER BY
    CASE WHEN b.binding_status = 'active' THEN 0 ELSE 1 END,
    b.created_at DESC
  LIMIT 1;
$$;

REVOKE ALL ON FUNCTION public.get_c3_key_source_oar_binding_support_read(
  text,
  text,
  text
) FROM PUBLIC;
...
GRANT EXECUTE ON FUNCTION public.get_c3_key_source_oar_binding_support_read(
  text,
  text,
  text
) TO service_role;
...
BEGIN
  ...
  IF v_reject_reason IS NULL
    ...
    IF v_binding_id IS NULL THEN
      v_hold_reason := 'active source / OAR binding is required';
    END IF;
  END IF;
  ...
  BEGIN
    ...
    SET
      ...
      metadata = metadata || jsonb_build_object(
        ...
        'source_oar_binding_validation', 'explicit_active_binding_verified',
