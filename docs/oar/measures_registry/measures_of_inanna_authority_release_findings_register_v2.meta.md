...
title: Measures of Inanna — Authority and Release Findings Register (Reconciled)
status: filed
version: v2
supersedes_for_reference: measures_of_inanna_authority_release_findings_register_v1.meta.md
operator: op044
...
correction_reason: several v1 findings carried more than one primary classification (joined by slash/conjunction). This file assigns exactly one primary classification to each finding, per the reconciliation OAR2's required standing table, moving all other facts into evidence/containment/consequence/dependency/missing-proof fields. No finding's underlying facts changed; only its classification structure did.
---

# Measures of Inanna — Authority and Release Findings Register (Reconciled)

A recommendation in this register is not authorization. No finding here has been remediated. Ten findings, reconciled from v1's ten, each now carrying exactly one primary classification.
...
- **Evidence:** `measures_release_state.phase_label` and `measures_phase_calendar.phase_key` share no common format; only 1 of 29 governed units (`gate_3_lapis_necklace`) ever matched. See [`measures_of_inanna_phase_cadence_audit_v2.meta.md`](measures_of_inanna_phase_cadence_audit_v2.meta.md) for the full, denominator-reconciled population table (replacing v1's unsupported "21 of 22").
...
- **Primary classification: `unresolved_pending_operator_decision`** (corrected from v1's dual `active_defect`/other framing).
...
- **Missing proof:** active caller reachability was not established — no invocation site was found for this function in either Audit 01 or this reconciliation.
...
- **Primary classification: `missing_authority`** (corrected from v1's dual `active_defect`/`missing_authority` framing).
...
- **Preserved gap (not resolved by this correction):** no single verified **operator-facing** read model was demonstrated that joins registry standing, branch completeness, assets, evidence, FREE readiness, and risk into one governed operational surface. The database views are a release/access layer specifically, not a comprehensive operational dashboard.
...
- **Primary classification: `historical_deprecated_residue`.** Unchanged from v1.
...
- **Primary classification: `unresolved_pending_operator_decision`.** Unchanged from v1 (this finding did not carry a multi-classification problem).
...
- **Primary classification: `missing_evidence`.** Unchanged from v1.
