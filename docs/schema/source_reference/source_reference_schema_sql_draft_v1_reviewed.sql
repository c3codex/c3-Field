-- Source Reference Schema SQL Draft v1 - Reviewed
-- Review standing: db_preflight_candidate, not executed
...
create or replace function public.mark_codex_source_reference_seeded(
  p_source_key text,
  p_oar_key text,
...
declare
  ...
  v_source_hash text;
begin
  if p_oar_key is null or btrim(p_oar_key) = '' then
    raise exception 'oar_key is required to mark a codex source reference seeded';
  end if;

  select id, source_status, source_path, source_hash
  into v_source_id, v_source_status, v_source_path, v_source_hash
  from public.codex_source_reference
  where source_key = p_source_key;

  if v_source_id is null then
    raise exception 'codex_source_reference not found: %', p_source_key;
  end if;

  if v_source_status = 'seeded' then
    raise exception 'codex_source_reference is already seeded: %', p_source_key;
  end if;

  if v_source_status <> 'committed' then
    raise exception 'codex_source_reference must be committed before seeded: %', p_source_key;
  end if;

  if v_source_path is null or v_source_hash is null then
    raise exception 'source_path and source_hash are required before seeded state: %', p_source_key;
  end if;

  update public.codex_source_reference
  set
    source_status = 'seeded',
    authority_level = 'codex_seated',
    readonly = true,
    seeded_at = now(),
    metadata = metadata || jsonb_build_object('seed_oar_key', p_oar_key)
...
revoke all on function public.mark_codex_source_reference_seeded(text, text, text, text) from public;
