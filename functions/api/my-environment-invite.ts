...
async function write(env:PassageEnv,table:string,method:"POST"|"PATCH",body:unknown,params:Record<string,string>={},prefer="return=representation"){
  ...
  const response=await fetch(url.toString(),{
    method,headers:headers(env,{"content-type":"application/json",prefer}),
...
async function verifiedInitiativeContext(env:PassageEnv,subjectKey:string,value:unknown){
  if(value===undefined||value===null||value==="") return null
  if(value!=="million_dollar_mission") throw new Error("initiative_context_not_resolved")
  const rows=await read(env,"c2_mdm_participation_relation",{
    select:"initiative_key,target_environment_key,standing,passage_standing,revoked_at",
    participant_relationship_key:"eq."+subjectKey,
  ...
  if(!relation||relation.passage_standing!=="c2_participation_granted")
    throw new Error("initiative_context_not_resolved")
  return {
    source_initiative_key:"million_dollar_mission",
    target_environment_key:relation.target_environment_key
  }
}
...
export const onRequestPost:PagesFunction<PassageEnv>=async({request,env})=>{
  try{
    ...
    if(origin&&origin!==new URL(request.url).origin)
      return json({ok:false,standing:"invite_origin_held"},403)
    ...
    const initiative=await verifiedInitiativeContext(env,session.subjectKey,body.source_initiative_key)

    const existing=await read(env,"c3_env_share_reference",{
      select:"share_reference,metadata",
      source_env_key:"eq."+session.envKey,
      source_envpac_key:"eq."+session.envpacKey,
      owner_subject_key:"eq."+session.subjectKey,
      share_state:"eq.active",
      limit:"1"
    })
    const now=new Date().toISOString()
    const metadata={
      reference_class:"opaque_encounter_provenance",
      source_process:"c1me_envpac_primitives_v1",
    ...
    if(existing[0]?.share_reference){
      shareReference=String(existing[0].share_reference)
      await write(env,"c3_env_share_reference","PATCH",{metadata,updated_at:now},{
        share_reference:"eq."+shareReference,
        source_envpac_key:"eq."+session.envpacKey,
        owner_subject_key:"eq."+session.subjectKey,
        share_state:"eq.active"
      },"return=minimal")
    }else{
      const rows=await write(env,"c3_env_share_reference","POST",{
      ...
      shareReference=String(rows[0].share_reference)
    }
