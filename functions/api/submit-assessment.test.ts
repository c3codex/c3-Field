...
test("resolves assessment standing server-side from seated mechanics", async () => {
  ...
  assert.match(resultEmail.text, /\/ai-operations-assessment\?evaluation=/)
  assert.doesNotMatch(resultEmail.text, /visit https:\/\/measuresregistry\.com\/map-the-environment/)
  ...
  assert.match(updatedCapture?.metadata.evaluation_delivery_token, /^[0-9a-f-]{36}$/i)
  assert.match(updatedCapture?.metadata.evaluation_delivery_url, /\/ai-operations-assessment\?evaluation=/)
  assert.equal(updatedCapture?.metadata.evaluation_delivery_permitted_encounter, "marble_chamber_orientation")
