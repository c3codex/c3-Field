...
export default function MyEnvironmentEncounter(){
  ...
  async function loadCanopy(){
    ...
    try{
      ...
      setCanopyMessage(error instanceof Error?error.message:"My Canopy is unavailable.")
      setCanopyState("held")
    }
  }
  ...
  async function saveCanopy(event:FormEvent<HTMLFormElement>){
    ...
    setCanopyBusy(editingReference||"add")
    setCanopyMessage("")
    try{
      await canopyAction(editingReference?{
        action:"edit",reference_key:editingReference,...canopyForm
      }:{
        action:"add",surface_label:canopyForm.surface_label,display_label:canopyForm.display_label,
        external_url:canopyForm.external_url,handle:canopyForm.handle,sort_order:canopyForm.sort_order
      })
      setCanopyForm(emptyCanopyForm)
      setEditingReference(null)
      setCanopyMessage(editingReference?"Canopy reference updated.":"Canopy reference added.")
      await loadCanopy()
    }catch(error){setCanopyMessage(error instanceof Error?error.message:"That Canopy action could not be completed.")}
    finally{setCanopyBusy(null)}
  }
  ...
  function editCanopy(reference:CanopyReference){
    ...
    setCanopyMessage("")
    document.getElementById("canopy-form")?.scrollIntoView({behavior:"smooth",block:"center"})
  }

  function cancelCanopyEdit(){
    setEditingReference(null)
    setCanopyForm(emptyCanopyForm)
    setCanopyMessage("")
  }
  ...
  async function removeCanopy(reference:CanopyReference){
    ...
    setCanopyBusy(reference.reference_key)
    setCanopyMessage("")
    try{
      await canopyAction({action:"remove",reference_key:reference.reference_key})
      if(editingReference===reference.reference_key) cancelCanopyEdit()
      setCanopyMessage("Canopy reference removed.")
      await loadCanopy()
    }catch(error){setCanopyMessage(error instanceof Error?error.message:"That Canopy reference could not be removed.")}
    finally{setCanopyBusy(null)}
  }
  ...
  async function openCanopy(reference:CanopyReference){
    ...
    setCanopyMessage("")
    try{
      const result=await canopyAction({action:"open",reference_key:reference.reference_key})
      if(typeof result.external_url!=="string") throw new Error("That Canopy destination is unavailable.")
      if(target) target.location.replace(result.external_url)
      else window.location.assign(result.external_url)
    }catch(error){
      target?.close()
      setCanopyMessage(error instanceof Error?error.message:"That Canopy destination could not be opened.")
    }finally{setCanopyBusy(null)}
  }
  ...
  async function copyC3Link(){
    ...
    setCanopyMessage("")
    try{
      const result=await canopyAction({action:"copy_c3_link"})
      if(typeof result.public_url!=="string") throw new Error("The public c3 link is unavailable.")
      await copyText(result.public_url)
      setCanopyMessage("c3 Connect link copied. Share it wherever you choose.")
    }catch(error){setCanopyMessage(error instanceof Error?error.message:"The c3 link could not be copied.")}
    finally{setCanopyBusy(null)}
  }
  ...
  return <main className="myenv-shell">
    <aside className="myenv-rail">
      ...
      <nav aria-label="My Environment"><a href="#overview" aria-current="page">Overview</a><a href="#canopy">My Canopy</a><a href="#systems">Rooted Systems</a><a href="#access">Access</a><span aria-disabled="true">Marketplace · HLD</span></nav>
    ...
    <section className="myenv-main" style={environmentStyle}>
      ...
      <section id="canopy" className="myenv-section myenv-canopy">
        ...
        {canopyState==="ready"&&<>
          <div className="myenv-canopy-list">
            {canopyReferences.length===0?<p className="myenv-canopy-empty">Your Canopy is empty. Add the first place you want to reach from your environment.</p>:canopyReferences.map(reference=><article className="myenv-canopy-card" key={reference.reference_key}>
              <div><p className="myenv-canopy-surface">{reference.surface_label}</p><h3>{reference.display_label||reference.surface_label}</h3>{reference.handle&&<span>{reference.handle}</span>}<small>{reference.external_url}</small></div>
              <div className="myenv-canopy-actions"><button type="button" className="myenv-primary" onClick={()=>void openCanopy(reference)} disabled={canopyBusy!==null}>Open</button><button type="button" className="myenv-secondary" onClick={()=>editCanopy(reference)} disabled={canopyBusy!==null}>Edit</button><button type="button" className="myenv-text-button" onClick={()=>void removeCanopy(reference)} disabled={canopyBusy!==null}>Remove</button></div>
            </article>)}
          </div>
          <form id="canopy-form" className="myenv-canopy-form" onSubmit={saveCanopy}>
            <div className="myenv-form-heading"><div><p className="myenv-kicker">Canopy Reference</p><h3>{editingReference?"Edit reference":"Add a place"}</h3></div>{editingReference&&<button type="button" className="myenv-text-button" onClick={cancelCanopyEdit}>Cancel</button>}</div>
            <div className="myenv-form-grid">
              <label><span>Surface</span><input required maxLength={80} placeholder="Facebook" value={canopyForm.surface_label} onChange={event=>setCanopyForm({...canopyForm,surface_label:event.target.value})}/></label>
              <label><span>Label</span><input maxLength={120} placeholder="My Facebook" value={canopyForm.display_label} onChange={event=>setCanopyForm({...canopyForm,display_label:event.target.value})}/></label>
              <label className="myenv-form-wide"><span>Destination</span><input required type="url" maxLength={2048} placeholder="https://…" value={canopyForm.external_url} onChange={event=>setCanopyForm({...canopyForm,external_url:event.target.value})}/></label>
              <label><span>Handle <small>optional</small></span><input maxLength={160} placeholder="@name" value={canopyForm.handle} onChange={event=>setCanopyForm({...canopyForm,handle:event.target.value})}/></label>
            </div>
            <div className="myenv-form-actions"><button type="submit" className="myenv-primary" disabled={canopyBusy!==null}>{canopyBusy?(editingReference?"Saving…":"Adding…"):(editingReference?"Save changes":"Add to My Canopy")}</button><span>No account password or provider authorization is requested.</span></div>
          </form>
          {canopyMessage&&<p className="myenv-canopy-status" role="status">{canopyMessage}</p>}
        </>}
      </section>
