...
export default function PassageMediaSurface({
  surfaceKey,
  title,
  onComplete,
}: PassageMediaSurfaceProps) {
  const { groups, loading, error } = useTempExhibitionMedia()
  const audioRefs = useRef<Record<number, HTMLAudioElement | null>>({})

  const items = useMemo(
    () => getSurfaceItems(groups, "passage", surfaceKey),
    [groups, surfaceKey]
  )
  ...
  if (loading) {
    return <div>Loading passage media…</div>
  }

  if (error) {
    return <div>Passage media error: {error}</div>
  }

  if (items.length === 0) {
    return <div>No passage media found for: {surfaceKey}</div>
  }

  return (
    <section
      data-surface-type="passage"
      ...
      {items.map((item) => {
        ...
        if (item.media_type === "audio") {
          return (
            ...
            >
              <div>{item.label ?? "Passage Audio"}</div>
              <audio
                ref={(node) => {
        ...
        return (
          <div key={item.id}>
            Unsupported passage media type: {item.media_type}
          </div>
        )
      })}
    </section>
